...
import { demoMovies as allMovies } from '../data/demo-content';
...
export function MoviesPage() {
  ...
  const {
    ...
    resultCount,
    totalCount,
  } = useContentControls(allMovies, 'movies');
      ...
      <main className="pt-32 pb-20">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          <div className="flex items-start justify-between mb-8">
            <div>
              <h1 className="text-5xl font-bold mb-4">فیلم‌ها</h1>
              <motion.p
                key={resultCount}
              ...
              >
                {resultCount === totalCount
                  ? `${totalCount.toLocaleString('fa-IR')} فیلم`
                  : `${resultCount.toLocaleString('fa-IR')} نتیجه از ${totalCount.toLocaleString('fa-IR')} فیلم`}
              </motion.p>
            </div>
          </div>
          ...
          {/* Movies Grid */}
          <ContentGrid items={items} layout={layout} baseUrl="/movie" />
        </div>
      </main>
